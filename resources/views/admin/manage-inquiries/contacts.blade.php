...
<div class="card" style="background-color: #efefef !important;">
    <div class="card-body">
        <div class="d-md-flex gap-4 align-items-center justify-content-between">
            <div class="fw-bold fs-4">Contact Submissions</div>
...
<div class="table-responsive mt-3">
    <table class="table table-custom table-lg mb-0" id="contacts-table">
        ...
        <tbody>
            ...
            <tr class="contact-row" data-contact-id="{{ $contact->id }}">
                ...
                <td>
                    <div class="d-flex flex-column gap-1">
                        <span><i class="bi bi-person-fill me-1"></i>{{ $contact->first_name }} {{ $contact->last_name }}</span>
                        <span><i class="bi bi-envelope-fill me-1"></i>{{ $contact->email }}</span>
                        <span><i class="bi bi-telephone-fill me-1"></i>{{ $contact->phone }}</span>
                    </div>
                </td>
                <td>{{ $contact->created_at->format('d M Y, h:i A') }}</td>
                <td class="text-end">
                    <form action="{{ route('admin.manage-contact.destroy', $contact->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this contact?')" style="display: inline;">
                        @csrf
                        @method('DELETE')
            ...
            <tr class="message-row d-none" data-contact-id="{{ $contact->id }}">
...
<script>
    $(document).ready(function () {
        ...
        function updateUrl() {
            ...
            window.location.href = "{{ route('admin.manage-contact.index') }}?page=1&records=" + recordsPerPage + "&sort=" + sortOption;
        }

        // Search logic across all cells
        $('#searchInput').on('input', function () {
            let searchText = $(this).val().toLowerCase();

            $('#contacts-table tbody tr.contact-row').each(function () {
                let row = $(this);
                let contactId = row.data('contact-id');
                let messageRow = $(`.message-row[data-contact-id="${contactId}"]`);
    ...
    $(document).on('click', '.toggle-btn', function () {
        ...
        const messageRow = $(`.message-row[data-contact-id="${contactId}"]`);
